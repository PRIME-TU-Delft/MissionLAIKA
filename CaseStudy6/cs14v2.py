...
class Lecture14Scene(PrimeScene, ThreeDScene):
    def construct(self):
        ...
        square = Square(color=yellow, fill_color=yellow, fill_opacity=0.5, side_length=unit_length,
                        shade_in_3d=True, ).shift(unit_length / 2 * (UP + RIGHT))
        ...
        x0_label3d = MathTex(r"\mathbf{x}_0", color=dark_blue, shade_in_3d=True).next_to(x0.get_center(), IN).rotate(90 * DEGREES,
                                                                                                     axis=RIGHT).rotate(
        ...
        self.move_camera(
            ...
            run_time=3,
            added_anims=[Write(cube), FadeOut(square), ReplacementTransform(x0_label2d, x0_label3d)]
        )
        ...
        a_label = \
            MathTex(r"\mathbf{a}", color=blue, shade_in_3d=True).next_to(a.get_center(), OUT).rotate(90 * DEGREES,
                                                                                                     axis=RIGHT).rotate(
                -45 * DEGREES, axis=np.array([0, 0, 1]))[0]
        b_label = \
            MathTex(r"\mathbf{b}", color=green, shade_in_3d=True).next_to(b.get_center(), OUT, ).rotate(90 * DEGREES,
                                                                                                        axis=RIGHT).rotate(
                -45 * DEGREES, axis=np.array([0, 0, 1]))[0]
        c_label = \
            MathTex(r"\mathbf{c}", color=red, shade_in_3d=True).next_to(c.get_center(), OUT, ).rotate(90 * DEGREES,
                                                                                                      axis=RIGHT).rotate(
                -45 * DEGREES, axis=np.array([0, 0, 1]))[0]
        ...
        self.play(ReplacementTransform(eq2[9:14], eq3[9:14]), eq2[:9].animate.set_opacity(0.2),
                  eq2[14:].animate.set_opacity(0.2))
        self.wait(1)

        ux0 = Arrow3D(start=axes.c2p(0, 0, 0), end=0.5 * (UP + 4*LEFT + OUT), color=dark_blue)
        ux0_label = \
            MathTex(r"\mathbf{u}(\mathbf{x}_0)", color=dark_blue, shade_in_3d=True, font_size=36).next_to(ux0.get_center(),
        ...
        self.play(ReplacementTransform(cube, cube_translated),
                  Group(a_label, b_label, c_label, a, b, c).animate.shift(0.5 * (UP + 4*LEFT + OUT)), GrowFromCenter(ux0))
        self.wait(1)
        ...
        Jc_label = \
            MathTex(r"J\mathbf{c}", color=red, shade_in_3d=True)[0].move_to(c_label.get_center()).rotate(
        ...
        Jc_label_2d = \
            MathTex(r"J\mathbf{c}", color=red, font_size=48)[0].shift(UNIT * RIGHT)
        ...
        self.move_camera(
            ...
            theta=-90 * DEGREES,
            run_time=3,
            added_anims=[ReplacementTransform(Ja_label, Ja_label_2d),
        ...
        self.play(ReplacementTransform(Ja_label_2d, eq4[1:3]),
                  ReplacementTransform(Jb_label_2d, eq4[5:7]),
                  ReplacementTransform(Jc_label_2d, eq4[8:10]),
                  GrowFromCenter(eq4[0]), GrowFromCenter(eq4[3:5]), GrowFromCenter(eq4[7]), GrowFromCenter(eq4[10:]))
        self.wait(1)
        ...
        self.play(LaggedStart(eq5.animate.shift(UNIT * UP), Write(eq6), Write(eq7), Write(eq8), lag_ratio=0.8))
        self.wait(1)
