...
class Lecture14Scene(PrimeScene, ThreeDScene):
    def construct(self):
        ...
        eq2 = MathTex(
            r"\text{Area} = |\operatorname{det} C| = 2",
            color=dark_blue, font_size=48)[0]
        eq2.shift(eq2[0].get_bottom()[1] * DOWN+ UNIT * UP + 3 * UNIT * RIGHT)

        small_grid = NumberPlane(background_line_style={
            "stroke_color": green,
            "stroke_width": 2,
            "stroke_opacity": 0.15
        },
            ...
            x_range=(-2, 2, 1),
            y_range=(-1, 2, 1),
        ).scale(UNIT).shift(1.5 * UNIT* DOWN)

        square = Square(side_length = UNIT, color=red, fill_color=red, fill_opacity=0.5,)
        ...
        self.play(Write(eq0))
        self.wait()
        self.play(LaggedStart(eq0.animate.shift(5*UNIT*LEFT), Write(eq1), lag_ratio=0.5))
        self.wait()
        self.play(Write(small_grid), Write( square), eq0.animate.shift(2*UNIT*UP), eq1.animate.shift(2*UNIT*UP))
        self.wait()
