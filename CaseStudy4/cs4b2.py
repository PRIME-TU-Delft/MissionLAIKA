...
class FlowAroundAirfoil(PrimeScene, MovingCameraScene):
    def construct(self):
        ...
        x_arrow = Line(
            start=ORIGIN, end=static_shell.get_center(), color=blue, stroke_width=2
        )
        x_arrow.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        x_arrow_label = MathTex(r"\mathbf{x}", color=blue, font_size=32).move_to(
            x_arrow.get_center() + 0.4 * DOWN
        )
        ...
        ux_arrow = Line(
            start=static_shell.get_center(),
            end=moving_shell.get_center(),
            color=dark_blue,
            stroke_width=2,
        )
        ux_arrow.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        ux_arrow_label = MathTex(
            r"\mathbf{u}( \mathbf{x}) - \mathbf{x}", color=dark_blue, font_size=32
        )[0].move_to(ux_arrow.get_center() + 0.5 * UP)
        ux_arrow_label[5].set_color(blue)
        ux_arrow_label[:4].set_color(yellow)
        xux_arrow = Line(
            start=ORIGIN, end=moving_shell.get_center(), color=yellow, stroke_width=2
        )
        xux_arrow.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        xux_arrow_label = MathTex(
            r"\mathbf{u}( \mathbf{x})", font_size=32, color=yellow
        )[0].move_to(xux_arrow.get_center() + LEFT)

        x0_arrow = Line(
            start=ORIGIN, end=static_shell.get_corner(DL), color=green, stroke_width=2
        )
        x0_arrow.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        x0_arrow_label = MathTex(r"\mathbf{x}_0", color=green, font_size=32).move_to(
            x_arrow.get_center() + 0.43 * DOWN
        )
        ...
        self.wait(1)
        self.play(
            Wiggle(static_shell, scale_value=1.5), Wiggle(moving_shell, scale_value=1.5)
        )
        self.wait(1)
        self.play(Wiggle(xux_arrow_label), Wiggle(ux_arrow_label[:4]))
        self.wait(1)
        self.play(
            FadeOut(moving_shell),
            FadeOut(ux_arrow),
            FadeOut(ux_arrow_label),
            FadeOut(x_arrow),
            FadeOut(x_arrow_label),
            FadeOut(xux_arrow),
            FadeOut(xux_arrow_label),
        )
        self.wait(1)
        self.play(LaggedStart(Write(x0_arrow), Write(x0_arrow_label), lag_ratio=0.5))
        self.wait(1)
        self.play(FadeOut(static_shell), FadeOut(x0_arrow), FadeOut(x0_arrow_label))
        self.play(
            self.camera.frame.animate.scale(2).shift(1 * DOWN + 0.55 * LEFT),
            arrow_field.animate.fade(0),
        )
        self.wait(1)
        ...
        self.play(
            FadeOut(airfoil_shape), FadeOut(arrow_field), FadeOut(grid1), FadeIn(grid2)
        )
        self.wait(1)
