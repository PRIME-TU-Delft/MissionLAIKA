...
class FlowAroundAirfoil(PrimeScene, MovingCameraScene):
    def construct(self):
        ...
        x0_arrow = Line(start=ORIGIN, end=static_shell.get_corner(DL), color=green, stroke_width=2)
        x0_arrow.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        x0_arrow_label = MathTex(r"\mathbf{x}_0", color=green, font_size=32).move_to(x_arrow.get_center() + 0.4 * DOWN)
        ...
        self.wait(1)
        self.play(Wiggle(static_shell, scale_value=1.5), Wiggle(moving_shell, scale_value=1.5))
        self.wait(1)
        ...
        self.play(self.camera.frame.animate.scale(2).shift(1 * DOWN + 0.55 * LEFT), arrow_field.animate.fade(0))
        self.wait(1)
        ...
        self.play(FadeOut(airfoil_shape), FadeOut(arrow_field), FadeOut(grid1), FadeIn(grid2))
        self.wait(1)
