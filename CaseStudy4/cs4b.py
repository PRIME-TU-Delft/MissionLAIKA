...
try:
    ...
    class VGroup: pass
    class VMobject: pass
    class StreamLines: pass
    class ArrowVectorField: pass
    BLUE = None
    YELLOW = None
...
class FlowAroundAirfoil(PrimeScene, MovingCameraScene):
    def construct(self):
        ...
        code = "2412"; n_pts = 201; U_inf = 1.0; alpha_deg = 0.0
        ...
        def vfield_func(pos):
            ...
            i = np.abs(xs - x).argmin(); j = np.abs(ys - y).argmin()
        ...
        sl = StreamLines(
            ...
            x_range=[-7, 9, 0.15], y_range=[-4, 4, 0.15],
            stroke_opacity=0.7, max_anchors_per_line=50,
            colors=[RED, BLUE, RED]
        )
...
if __name__ == "__main__":
    code = "0012"; pts = naca4_coordinates(code, n=101, chord=4.0)
    xv, yv, xc, yc, nx, ny = build_vortex_collocation(pts)
    A = build_A(xv, yv, xc, yc, nx, ny)
