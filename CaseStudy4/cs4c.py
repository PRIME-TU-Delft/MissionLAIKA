...
def naca4_coordinates(code: str, n: int = 201, chord: float = 1.0,
                      cosine: bool = True, closed: bool = True) -> np.ndarray:
    m = int(code[0]) / 100.0
    p = int(code[1]) / 10.0
    t = int(code[2:]) / 100.0
    x = cosine_spacing(n) if cosine else np.linspace(0.0, 1.0, n)
    yt = 5 * t * (0.2969 * np.sqrt(np.maximum(x, 1e-12)) - 0.1260 * x - 0.3516 * x**2 + 0.2843 * x**3 - 0.1015 * x**4)
    yc = np.zeros_like(x)
...
class Lecture4Scene(PrimeScene):
    def construct(self):
        ...
        x = MathTex(r"\mathbf{x} ="
            r"\begin{bmatrix} x_1\\ x_2 \end{bmatrix}", color=dark_blue, font_size=60)[0]
        x.shift(3 * UNIT * LEFT + 3 * UNIT * UP + x[0].get_bottom()[1] * DOWN)
        x[0].set_color(blue)
        x[3:7].set_color(blue)

        ux = MathTex(r"\mathbf{u} ="
            r"\begin{bmatrix} u_1(\mathbf{x})\\ u_2(\mathbf{x}) \end{bmatrix}", color=dark_blue, font_size=60)[0]
        ...
        eq1 = MathTex(r"u_1(\mathbf{x}) \approx u_1(\mathbf{x}_0) + \nabla u_1(\mathbf{x}_0) \cdot (\mathbf{x}-\mathbf{x}_0)", color=dark_blue, font_size=60)[0]
        ...
        eq3 = MathTex(r"\begin{bmatrix}u_1 (\mathbf{x})\\u_2  (\mathbf{x})\end{bmatrix} \approx"
        ...
        self.play(LaggedStart(ReplacementTransform(eq1[0:5], eq3[1:6]),
                              ReplacementTransform(eq2[0:5], eq3[6:11]),
                              Write(eq3[:1]), Write(eq3[11:12]),
                              ...
                              Write(eq3[13:14]), Write(eq3[26:27]),
                              ...
                              Write(eq3[28]), Write(eq3[73]),
                              lag_ratio=0.5))

        self.remove(eq3xcopy)
        self.wait(1)

        eq4 = MathTex(r"\mathbf{u} (\mathbf{x}) \approx \mathbf{u}(\mathbf{x}_0)+"
                      r"\begin{bmatrix}\frac{\partial u_1}{\partial x_1}(\mathbf{x}_0) & "
                      r"\frac{\partial u_1}{\partial x_2}(\mathbf{x}_0)\\\frac{\partial u_2}{\partial x_1}(\mathbf{x}_0)"
                      r" & \frac{\partial u_2}{\partial x_2}(\mathbf{x}_0)\end{bmatrix} (\mathbf{x}-\mathbf{x}_0)",
                      color=dark_blue, font_size=60)[0]
        ...
        self.play(LaggedStart(ReplacementTransform(eq3[:12], eq4[:4]),
        ...
        eq4a = MathTex(r"J = \frac{\partial(u_1, u_2)}{\partial(x_1, x_2)} = \begin{bmatrix}\frac{\partial u_1}{\partial x_1}(\mathbf{x}_0) & "
                       r"\frac{\partial u_1}{\partial x_2}(\mathbf{x}_0)\\\frac{\partial u_2}{\partial x_1}(\mathbf{x}_0)"
        ...
        self.play(ReplacementTransform(eq4[11:57].copy(), eq4a[len(eq4a) - 46:]),
                              Write(eq4a[:len(eq4a) - 46]))
        self.wait(1)

        eq5 = MathTex(r"\mathbf{u}(\mathbf{x}) \approx \mathbf{u}(\mathbf{x}_0) + J (\mathbf{x}-\mathbf{x}_0)",
                      color=dark_blue, font_size=60)[0]
        ...
        self.play(LaggedStart(ReplacementTransform(eq4[11:57], eq5[11]),
        ...
        eq8 = MathTex(r"\mathbf{u}(\mathbf{x}_0+\Delta \mathbf{x}) \approx \mathbf{u}(\mathbf{x}_0) + J (\mathbf{x}_0+\Delta \mathbf{x}-\mathbf{x}_0)=\mathbf{u}(\mathbf{x}_0) + J \Delta \mathbf{x}",
                      color=dark_blue, font_size=48)[0]
        ...
        self.play(Write(eq6))
        self.wait(1)
        self.play(LaggedStart(ReplacementTransform(eq6, eq6b[:2]), Write(eq6b[2:]), lag_ratio=0.5))
        self.wait(1)
        self.play(Write(eq7))
        self.wait(1)
        self.play(eq7.animate.shift(3*UNIT*LEFT), eq6b.animate.shift(3*UNIT*RIGHT + 3*UNIT*UP))
        self.wait(1)
        self.play(Write(eq8))
        self.wait(1)
        self.play(LaggedStart(ShrinkToCenter(eq8[9:27]), ReplacementTransform(eq8[:9], eq9[:9]),
