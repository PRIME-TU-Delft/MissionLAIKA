...
class Lecture4Scene(PrimeScene, MovingCameraScene):
    def construct(self):
        ...
        # Plane body
        plane = SVGMobject("plane.svg", stroke_color=dark_blue, stroke_width=4).scale(0.53).shift(UP*0.33 + LEFT*0.13)
        ax = Axes(x_range=[-5, 5, 1], y_range=[-3, 3, 0.5], x_length=14, y_length=8)

        # --- Replace ellipse wing with real airfoil ---
        airfoil_pts = naca4_coordinates("2412", n=201, chord=1.5)  # Adjust chord to size wing
        airfoil_shape = VMobject(color=dark_blue, stroke_width=4)
        airfoil_shape.set_points_smoothly([ax.coords_to_point(px, py) for px, py in airfoil_pts])
        airfoil_shape.shift(DOWN * 0.15 + LEFT * 0.13).scale(0.4)  # Adjust position & size
        ...
        self.play(LaggedStart(Unwrite(plane), airfoil_shape.animate.scale(1/0.4).shift(UP * 0.15 + RIGHT * 0.13).scale(4/1.5), lag_ratio=0.5))
        self.wait(1)
