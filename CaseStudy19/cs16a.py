...
class Plane(MovingCameraScene, PrimeScene):
    def construct(self):
        ...
        airplane_svg.set_color(dark_blue).set_stroke(color=dark_blue, width=12).scale(
            2
        ).shift(5.5 * UNIT * LEFT + UP)
        ...
        self.play(
            self.camera.frame.animate.scale(1 / 1.5).shift(
                5.5 * UNIT * RIGHT + 0.05 * DOWN
            )
        )
        self.wait()

        airplane_half = SVGMobject("../assets/AirplaneFrontHalf.svg")
        airplane_half.set_color(dark_blue).set_stroke(color=dark_blue, width=12).scale(
            2
        ).shift(UP).align_to(airplane_svg, RIGHT)
        ...
        for k in range(8):
            hatch_lines.append(
                Line(
                    start=(wall.get_right() + UNIT * LEFT)
                    + (-1.5 * UNIT + (3 * UNIT / 8) * k) * UP,
                    end=wall.get_right()
                    + (-1.5 * UNIT + (3 * UNIT / 8) * (k + 1)) * UP,
                    color=dark_blue,
                    stroke_width=3,
                )
            )
        ...
        eq1 = MathTex(r"mx'' + kx = 0", color=dark_blue, font_size=32)[0]
        ...
        cloud.set_color(dark_blue).set_stroke(color=dark_blue, width=3).shift(
            6 * UNIT * RIGHT + 4 * UNIT * UP
        )
        cloud_circle1 = (
            Circle(stroke_color=dark_blue, stroke_width=3)
            .scale(0.1 * UNIT)
            .stretch(1.2, 0)
            .shift(2.75 * UNIT * RIGHT + 3.25 * UNIT * UP)
        )
        cloud_circle2 = (
            Circle(stroke_color=dark_blue, stroke_width=3)
            .scale(0.2 * UNIT)
            .stretch(1.2, 0)
            .shift(3.5 * UNIT * RIGHT + 3.5 * UNIT * UP)
        )

        self.play(
            LaggedStart(
                FadeIn(cloud_circle1),
                FadeIn(cloud_circle2),
                FadeIn(cloud),
                FadeIn(eq1),
                lag_ratio=0.2,
            )
        ...
        self.play(
            LaggedStart(
                FadeOut(cloud_circle1),
                FadeOut(cloud_circle2),
                FadeOut(cloud),
                FadeOut(eq1),
                lag_ratio=0.2,
            )
        ...
        p1 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.5 * beam_width * RIGHT
        )
        ...
        def dof_glyphs(anchor: Mobject, idx: str) -> VGroup:
            ...
            tx = MathTex(rf"u_{idx}", font_size=36, color=yellow).next_to(
                ax, RIGHT, buff=0.12
            )

            ay = Arrow(base, base + 0.85 * UP, color=yellow, stroke_width=4, buff=0.0)
            ty = MathTex(rf"v_{idx}", font_size=36, color=yellow).next_to(
                ay, UP, buff=0.12
            )
            ...
            ttheta = (
                MathTex(rf"\theta_{idx}", font_size=36, color=yellow)
                .next_to(arc, LEFT, buff=0.2)
                .shift(0.2 * UP)
            )
        ...
        self.play(
            LaggedStart(
                FadeOut(VGroup(wall, *hatch_lines, beam, eq2, dofs1, dofs2, p1, p2)),
                eq0.animate.shift(7 * UNIT * LEFT + 4 * UNIT * DOWN),
                lag_ratio=0.5,
            )
        )
        self.wait()
        ...
        self.play(
            ReplacementTransform(eq0[:4], eq3[3:7]),
            ReplacementTransform(eq0[4:7], eq3[10:13]),
            ReplacementTransform(eq0[7:], eq3[16:]),
            GrowFromCenter(eq3[:3]),
            GrowFromCenter(eq3[7:10]),
            GrowFromCenter(eq3[13:16]),
        )
        self.wait()
        ...
        self.play(
            ReplacementTransform(eq3[4:13], eq4[:9]),
        ...
        eq5 = MathTex(r"A = M^{-1}K", color=dark_blue, font_size=48)[0]
        ...
        eq20 = MathTex(r"M", color=dark_blue, font_size=48)[0]
        eq20.shift(eq20[0].get_bottom()[1] * DOWN + 16 * UNIT * RIGHT)

        eq21 = MathTex(r"K", color=dark_blue, font_size=48)[0]
        ...
        self.play(
            LaggedStart(GrowFromCenter(eq20), GrowFromCenter(eq21), lag_ratio=0.5)
        )
        self.wait()

        self.play(
            LaggedStart(
                ReplacementTransform(eq20, eq20a[0]),
                Write(eq20a[1:]),
        ...
        eq22 = MathTex(r"\left(M^{-1}\right)^T=M^{-1}", color=dark_blue, font_size=48)[
            0
        ]
        ...
        eq23 = MathTex(r"(M^{-1}K)^T\neq M^{-1}K", color=dark_blue, font_size=48)[0]
        ...
        eq26a = MathTex(r"\mathbf{x}^TM\mathbf{x}", color=dark_blue, font_size=48)[0]
        eq26a.shift(eq26a[0].get_bottom()[1] * DOWN + 19 * UNIT * RIGHT)

        self.play(
            Group(eq24, eq25).animate.shift(3 * UNIT * UP),
            ReplacementTransform(eq24[13:17].copy(), eq26a),
        )
        self.wait()
        ...
        self.play(
            ShrinkToCenter(eq26a[2]),
            ReplacementTransform(eq25[2:].copy(), eq26b[6 : 6 + len(eq25[2:])]),
            eq26a[3].animate.shift(2 * UNIT * RIGHT),
        )
        ...
        self.play(
            ReplacementTransform(eq26b, eq26c[: len(eq26b)]), Write(eq26c[len(eq26b) :])
        )
        self.wait()
        ...
        eq12 = MathTex(r"\mathbf{y} = P^{-1}\mathbf{u}", color=dark_blue, font_size=48)[
            0
        ]
        ...
        self.play(
            LaggedStart(
                eq11.animate.shift(7 * UNIT * UP + 7 * UNIT * LEFT),
                eq10.animate.shift(3 * UNIT * DOWN + 1 * UNIT * RIGHT),
                FadeOut(VGroup(eq7, eq8)),
                Write(eq12[:2]),
                ReplacementTransform(eq9[0], eq12[2]),
                FadeOut(eq9[1:]),
                Write(eq12[3:]),
                lag_ratio=0.5,
            )
        ...
        self.play(
            eq12.animate.shift(1 * UNIT * UP),
            eq6.animate.shift(5 * UNIT * DOWN + 7 * UNIT * LEFT),
        )
        self.wait()
        ...
        self.play(
            ReplacementTransform(eq6[:3], eq13[3:6]),
        ...
        self.play(
            ReplacementTransform(eq13[:12], eq13b[:12]),
        ...
        self.play(
            ReplacementTransform(eq13b[:9], eq14[:9]),
            ReplacementTransform(eq11[2:].copy(), eq14[9:14]),
            ShrinkToCenter(eq13b[9]),
            ReplacementTransform(eq13b[10:], eq14[14:]),
        )
        self.wait()
        ...
        self.play(
            ReplacementTransform(eq14[:6], eq15[:6]),
            ShrinkToCenter(eq14[6:10]),
            ReplacementTransform(eq14[10:], eq15[6:]),
        )
        self.wait()

        self.play(
            Wiggle(VGroup(eq15[:3], eq15[4])), Wiggle(eq15[7:11]), Wiggle(eq12[2:])
        )
        self.wait()
        ...
        self.play(
            ReplacementTransform(VGroup(*eq15[:3], eq15[4]), eq16[1]),
            ReplacementTransform(eq15[3], eq16[0]),
            ReplacementTransform(eq15[5:7], eq16[2:4]),
            ReplacementTransform(eq15[7:11], eq16[4]),
            ReplacementTransform(eq15[11:], eq16[5:]),
        )
        self.wait()

        self.play(
            eq12.animate.shift(3 * UNIT * UP + 7 * UNIT * RIGHT),
            eq16.animate.shift(1 * UNIT * UP),
        )
        self.wait()

        self.play(
            LaggedStart(
                Indicate(eq10[5:8], color=green, scale_factor=1.2),
                ...
                Indicate(eq10[17:20], color=green, scale_factor=1.2),
                lag_ratio=0.25,
            )
        )
        self.wait()
        ...
        eq17 = MathTex(
            r"\ddot y_1 + \omega_1^2 y_1 = 0", color=dark_blue, font_size=48
        )[0]
        eq17.shift(eq17[4].get_bottom()[1] * DOWN + 2 * UNIT * UP)
        eq18 = MathTex(
            r"\ddot y_2 + \omega_2^2 y_2 = 0", color=dark_blue, font_size=48
        )[0]
        eq18.shift(eq18[4].get_bottom()[1] * DOWN + UNIT * UP)
        eq19 = MathTex(
            r"\ddot y_3 + \omega_3^2 y_3 = 0", color=dark_blue, font_size=48
        )[0]
        ...
        eq21 = MathTex(
            r"\ddot y_n + \omega_n^2 y_n = 0", color=dark_blue, font_size=48
        )[0]
        eq21.shift(eq21[4].get_bottom()[1] * DOWN + 2 * UNIT * DOWN)

        self.play(
            LaggedStart(
                ReplacementTransform(eq16[0].copy(), eq17[0]),
                ReplacementTransform(eq16[1].copy(), eq17[1]),
                ReplacementTransform(eq16[2].copy(), eq17[3]),
                ReplacementTransform(eq16[3].copy(), eq17[4]),
                ReplacementTransform(eq16[4].copy(), eq17[6]),
                ReplacementTransform(eq16[5:].copy(), eq17[8:]),
                Write(eq17[2]),
                Write(eq17[5]),
                Write(eq17[7]),
        ...
        p2 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.1 * beam_width * RIGHT
        )
        p3 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.2 * beam_width * RIGHT
        )
        p4 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.3 * beam_width * RIGHT
        )
        p5 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.4 * beam_width * RIGHT
        )
        p6 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.5 * beam_width * RIGHT
        )
        p7 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.6 * beam_width * RIGHT
        )
        p8 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.7 * beam_width * RIGHT
        )
        p9 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.8 * beam_width * RIGHT
        )
        p10 = Dot(color=blue, radius=0.085).move_to(
            beam.get_left() + 0.9 * beam_width * RIGHT
        )

        self.play(
            LaggedStart(
                *[FadeIn(p) for p in [p2, p3, p4, p5, p6, p7, p8, p9, p10, p1]],
                lag_ratio=0.2,
            )
        )
        self.wait()
