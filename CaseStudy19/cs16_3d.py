...
class Lecture14Scene(PrimeScene, ThreeDScene):
    def construct(self):
        ...
        self.move_camera(
            ...
            added_anims=[
                LaggedStart(
                    grid2.animate.scale(unit_length / UNIT), Write(axes), lag_ratio=0.5
                )
            ],
        )
        self.wait(1)
        ...
        surface = Surface(
            lambda u, v: axes.c2p(u, v, 3 * u**2 - 2 * u * v + 2 * v**2),
            u_range=[-3, 3],
            v_range=[-3, 3],
            resolution=(40, 40),
        )
        ...
        surface.set_style(
            fill_opacity=0.5, stroke_color=red, stroke_width=0.5, fill_color=red
        )
