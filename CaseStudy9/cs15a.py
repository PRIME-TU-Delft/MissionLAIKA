...
class Plane(MovingCameraScene, PrimeScene):
    def construct(self):
        ...
        # vertical movement
        self.play(
            left_square.animate.shift(2 * UNIT * UP),
            right_square.animate.shift(2 * UNIT * DOWN),
            run_time=0.5,
        )
        self.play(
            left_square.animate.shift(4 * UNIT * DOWN),
            right_square.animate.shift(4 * UNIT * UP),
            run_time=0.5,
        )
        self.play(
            left_square.animate.shift(2 * UNIT * UP),
        ...
        self.play(
            LaggedStart(
                Write(m1_left),
                left_square.animate.set_color(red),
                Write(m1_right),
                right_square.animate.set_color(red),
                lag_ratio=0.2,
            )
        )
        self.wait(1)
        ...
        self.play(
            LaggedStart(
                GrowFromPoint(k_left, 3 * LEFT * UNIT),
        ...
        x_axis = Line(
            start=10 * UNIT * LEFT + 5 * UNIT * DOWN,
            end=8 * UNIT * RIGHT + 5 * UNIT * DOWN,
            color=dark_blue,
            stroke_width=4,
        )
        x_axis.add_tip()
        x_axis_label = MathTex("x", font_size=60, color=dark_blue).move_to(
            x_axis.get_right() + 0.5 * UNIT * DOWN
        )
        ...
        x1_line = DashedLine(
            left_square.get_bottom(),
            left_square.get_bottom() + 2 * DOWN * UNIT,
            color=yellow,
            dash_length=0.2 * UNIT,
            dashed_ratio=0.6,
        )
        x2_line = DashedLine(
            center_square.get_bottom(),
            center_square.get_bottom() + DOWN * UNIT,
            color=yellow,
            dash_length=0.2 * UNIT,
            dashed_ratio=0.6,
        )
        x3_line = DashedLine(
            right_square.get_bottom(),
            right_square.get_bottom() + 2 * DOWN * UNIT,
            color=yellow,
            dash_length=0.2 * UNIT,
            dashed_ratio=0.6,
        )
        ...
        self.play(
            LaggedStart(Write(x1_line), Write(x2_line), Write(x3_line), lag_ratio=0.2)
        )
        ...
        x1_dist = always_redraw(
            lambda: Line(x1_equil, x1_line.get_bottom(), color=yellow)
        )
        x2_dist = always_redraw(
            lambda: Line(x2_equil, x2_line.get_bottom(), color=yellow)
        )
        x3_dist = always_redraw(
            lambda: Line(x3_equil, x3_line.get_bottom(), color=yellow)
        )
        self.play(Write(x1_dist), Write(x2_dist), Write(x3_dist))

        x1 = MathTex("x_1", font_size=60, color=yellow).move_to(
            x1_dist.get_center() + 0.5 * UNIT * DOWN
        )
        x2 = MathTex("x_2", font_size=60, color=yellow).move_to(
            x2_dist.get_center() + 0.5 * UNIT * DOWN
        )
        x3 = MathTex("x_3", font_size=60, color=yellow).move_to(
            x3_dist.get_center() + 0.5 * UNIT * DOWN
        )
        ...
        left_square.add_updater(
            lambda m: m.move_to(
                RIGHT * UNIT * (x1_tracker.get_value()) + LEFT * UNIT * 5
            )
        )
        m1_left.add_updater(lambda m: m.move_to(left_square.get_center()))
        x1_line.add_updater(
            lambda m: m.move_to(left_square.get_center() + 2 * DOWN * UNIT)
        )
        ...
        center_square.add_updater(
            lambda m: m.move_to(RIGHT * UNIT * (x2_tracker.get_value()))
        )
        m2.add_updater(lambda m: m.move_to(center_square.get_center()))
        x2_line.add_updater(
            lambda m: m.move_to(center_square.get_center() + 2.5 * DOWN * UNIT)
        )
        ...
        right_square.add_updater(
            lambda m: m.move_to(
                RIGHT * UNIT * (x3_tracker.get_value()) + RIGHT * UNIT * 5
            )
        )
        m1_right.add_updater(lambda m: m.move_to(right_square.get_center()))
        x3_line.add_updater(
            lambda m: m.move_to(right_square.get_center() + 2 * DOWN * UNIT)
        )
        ...
        k_left.add_updater(
            lambda m: m.move_to(spring_left.get_center() + 1.25 * UP * UNIT)
        )
        k_right.add_updater(
            lambda m: m.move_to(spring_right.get_center() + 1.25 * UP * UNIT)
        )

        self.play(
            x1_tracker.animate.set_value(2), x2_tracker.animate.set_value(1), run_time=2
        )
        self.play(
            x1_tracker.animate.set_value(0), x2_tracker.animate.set_value(0), run_time=2
        )
        self.play(
            x3_tracker.animate.set_value(-2),
            x2_tracker.animate.set_value(-1),
            run_time=2,
        )
        self.play(
            x3_tracker.animate.set_value(0), x2_tracker.animate.set_value(0), run_time=2
        )
        self.play(
            x1_tracker.animate.set_value(-3),
            x2_tracker.animate.set_value(-2),
            x3_tracker.animate.set_value(-1),
            run_time=2,
        )
        self.play(
            x1_tracker.animate.set_value(0),
            x2_tracker.animate.set_value(0),
            x3_tracker.animate.set_value(0),
            run_time=2,
        )
        self.wait(2)
        ...
        eq01 = MathTex("x_2-x_1", color=dark_blue, font_size=60)[0].shift(
            2.75 * UNIT * DOWN + 3 * UNIT * LEFT
        )
        eq01[:2].set_color(yellow)
        eq01[3:].set_color(yellow)
        eq02 = MathTex("-k(x_2-x_1)", color=dark_blue, font_size=60)[0].shift(
            2.75 * UNIT * DOWN + 3 * UNIT * LEFT
        )
        eq02[1].set_color(blue)
        eq02[3:5].set_color(yellow)
        eq02[6:8].set_color(yellow)
        eq03 = MathTex("k(x_3-x_2)", color=dark_blue, font_size=60)[0].shift(
            2.75 * UNIT * DOWN + 3 * UNIT * RIGHT
        )
        eq03[0].set_color(blue)
        eq03[2:4].set_color(yellow)
        eq03[5:7].set_color(yellow)
        eq0 = MathTex("F_2 = -k(x_2-x_1) + k(x_3-x_2)", color=dark_blue, font_size=60)[
            0
        ...
        self.wait(1)
        self.play(
            ReplacementTransform(eq01, eq02[3:8]), Write(eq02[:3]), Write(eq02[8])
        )
        self.wait(1)
        ...
        self.wait(1)
        self.play(
            ReplacementTransform(eq02, eq0[3 : 3 + len(eq02)]),
            ReplacementTransform(eq03, eq0[13:]),
            Write(eq0[:3]),
            Write(eq0[12]),
        )
        self.wait(1)
        ...
        self.play(
            eq0.animate.shift(9 * UNIT * LEFT + 4 * UNIT * DOWN - eq0[0].get_corner(DL))
        )
        self.wait(1)
        ...
        eq21 = MathTex("--k(x_2-x_1)", color=dark_blue, font_size=60)[0].shift(
            2.75 * UNIT * DOWN + 3 * UNIT * LEFT
        )
        eq21[2].set_color(blue)
        eq21[4:6].set_color(yellow)
        eq21[7:9].set_color(yellow)
        eq2 = MathTex("F_1 = k(x_2-x_1)", color=dark_blue, font_size=60)[0].shift(
            2.75 * UNIT * DOWN + 3.5 * UNIT * LEFT
        )
        ...
        self.play(
            eq2.animate.shift(9 * UNIT * LEFT + 3 * UNIT * DOWN - eq2[0].get_corner(DL))
        )
        self.wait(1)
        ...
        eq31 = MathTex("-k(x_3-x_2)", color=dark_blue, font_size=60)[0].shift(
            2.5 * UNIT * RIGHT, 2.75 * UNIT * DOWN
        )
        eq31[1].set_color(blue)
        eq31[3:5].set_color(yellow)
        eq31[6:8].set_color(yellow)
        eq3 = MathTex("F_3 = -k(x_3-x_2)", color=dark_blue, font_size=60)[0].shift(
            2.5 * UNIT * RIGHT, 2.75 * UNIT * DOWN
        )
        ...
        self.play(
            eq3.animate.shift(9 * UNIT * LEFT + 5 * UNIT * DOWN - eq3[0].get_corner(DL))
        )
        self.wait(1)
        ...
        self.play(
            LaggedStart(eq4.animate.shift(3 * UNIT * LEFT), Write(eq5), lag_ratio=0.4)
        )
        self.wait(1)
        ...
        self.play(
            ShrinkToCenter(eq4[3]),
            ShrinkToCenter(eq5[:2]),
            ReplacementTransform(eq4[:3], eq6[:3]),
            ReplacementTransform(eq5[2:], eq6[3:]),
        )

        self.play(eq6.animate.scale(0.6))
        self.play(
            eq6.animate.shift(3 * UNIT * UP + 8 * RIGHT * UNIT - eq6.get_bottom())
        )
        ...
        self.play(
            GrowFromCenter(eq8[6]),
            GrowFromCenter(eq8[9]),
            GrowFromCenter(eq8[12]),
        ...
        self.play(
            GrowFromCenter(eq9[2]),
            GrowFromCenter(eq9[8]),
            GrowFromCenter(eq9[12]),
            GrowFromCenter(eq9[16]),
        ...
        self.play(
            LaggedStart(
                eq6.animate.shift(4 * UNIT * UP - eq6.get_bottom()),
                eq9.animate.shift(
                    5 * UNIT * RIGHT
                    + 4 * UNIT * UP
                    - eq9[0].get_bottom()[1] * UP
                    - eq9.get_center()[0] * RIGHT
                ),
        ...
        eq10[5 + len(eq2) + len(eq0) + 6 : 5 + len(eq2) + len(eq0) + 8].set_color(
            yellow
        )
        eq10[5 + len(eq2) + len(eq0) + 9 : 5 + len(eq2) + len(eq0) + 11].set_color(
            yellow
        )

        self.play(ReplacementTransform(eq2, eq10[5 : 5 + len(eq2)]))
        self.play(
            ReplacementTransform(eq0, eq10[5 + len(eq2) : 5 + len(eq2) + len(eq0)])
        )
        self.play(
            ReplacementTransform(
                eq3, eq10[5 + len(eq2) + len(eq0) : 5 + len(eq2) + len(eq0) + len(eq3)]
            )
        )
        ...
        eq11[
            5 + len(eq2) + len(eq0) + 6 + 12 : 5 + len(eq2) + len(eq0) + 8 + 12
        ].set_color(yellow)
        eq11[
            5 + len(eq2) + len(eq0) + 9 + 12 : 5 + len(eq2) + len(eq0) + 11 + 12
        ].set_color(yellow)
        eq11[5:7].set_color(red)
        eq11[5 + len(eq2) + 4 : 5 + len(eq2) + 4 + 2].set_color(red)
        eq11[
            5 + len(eq2) + 4 + len(eq0) + 4 : 5 + len(eq2) + 4 + len(eq0) + 4 + 2
        ].set_color(red)
        ...
        self.play(
            ReplacementTransform(eq10[:5], eq11[:5]),
            ReplacementTransform(
                eq10[7 : 7 + len(eq2) - 2], eq11[11 : 11 + len(eq2) - 2]
            ),
            ReplacementTransform(
                eq10[7 + len(eq2) : 7 + len(eq2) + len(eq0) - 2],
                eq11[11 + len(eq2) + 4 : 11 + len(eq2) + len(eq0) + 2],
            ),
            ReplacementTransform(
                eq10[7 + len(eq2) + len(eq0) : 7 + len(eq2) + len(eq0) + len(eq3) - 2],
                eq11[
                    11 + len(eq2) + len(eq0) + 8 : 11
                    + len(eq2)
                    + len(eq0)
                    + len(eq3)
                    + 6
        ...
        self.play(
            ReplacementTransform(eq6[2].copy(), eq11[5]),
            ...
            ReplacementTransform(
                eq6[3:].copy(), eq11[5 + len(eq2) + 4 + 2 : 5 + len(eq2) + 4 + 2 + 3]
            ),
            ReplacementTransform(
                eq6[3:].copy(),
                eq11[
                    5 + len(eq2) + 4 + len(eq0) + 4 + 2 : 5
                    + len(eq2)
                    + 4
                    + len(eq0)
                    + 4
            ...
            ReplacementTransform(
                eq10[5 + len(eq2) + 1].copy(), eq11[5 + len(eq2) + 4 + 1]
            ),
            ReplacementTransform(
                eq10[5 + len(eq2) + len(eq0) + 1].copy(),
                eq11[5 + len(eq2) + 4 + len(eq0) + 4 + 1],
            ),
            ReplacementTransform(eq10[5 + 1], eq11[5 + 2 + 3]),
            ReplacementTransform(
                eq10[5 + len(eq2) + 1], eq11[5 + len(eq2) + 4 + 2 + 3]
            ),
            ReplacementTransform(
                eq10[5 + len(eq2) + len(eq0) + 1],
                eq11[5 + len(eq2) + 4 + len(eq0) + 4 + 2 + 3],
            ),
        )
