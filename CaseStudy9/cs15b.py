...
class Plane(MovingCameraScene, PrimeScene):
    def construct(self):
        ...
        cloud = SVGMobject(
            "../assets/cloud.svg", stroke_color=dark_blue, stroke_width=3
        )
        ...
        cloud_circle1 = (
            ...
            .scale(0.1 * UNIT)
            .stretch(1.2, 0)
            .shift(6 * UNIT * LEFT + 1.4 * UNIT * UP)
        )
        cloud_circle2 = (
            Circle(stroke_color=dark_blue, stroke_width=3)
            .scale(0.2 * UNIT)
            .stretch(1.2, 0)
            .shift(5.6 * UNIT * LEFT + 2 * UNIT * UP)
        )

        eq2 = MathTex(
            r"A \textbf{x} = \lambda \textbf{x}", color=dark_blue, font_size=60
        )[0].shift(3 * UNIT * UP + 3 * UNIT * LEFT)
        self.play(
            LaggedStart(
        ...
        self.wait(1)
        self.play(
            FadeOut(eq2), FadeOut(cloud_circle1), FadeOut(cloud_circle2), FadeOut(cloud)
        )
        self.wait(1)
        ...
        self.play(
            self.camera.frame.animate.move_to(18 * UNIT * RIGHT + 2.5 * UNIT * DOWN)
        )
        self.wait(1)

        question = MathTex(
            r"\text{Which function } y = f(t) \text{ becomes a multiple}",
            color=dark_blue,
            font_size=42,
        )[0]
        question2 = MathTex(
            r"\text{of itself after differentiating twice?}",
            color=dark_blue,
            font_size=42,
        )[0]
        ...
        self.play(
            ReplacementTransform(eq5[0], eq6[0]),
        ...
        self.wait(0.2)
        self.play(
            ReplacementTransform(eq8, eq9[: len(eq8)]), GrowFromCenter(eq9[len(eq8) :])
        )
        self.wait(0.2)

        # REVERSE
        self.play(
            ReplacementTransform(eq9[: len(eq8)], eq8), ShrinkToCenter(eq9[len(eq8) :])
        )
        self.play(
        ...
        self.wait(1)
        self.play(
            self.camera.frame.animate.move_to(2 * UNIT * DOWN),
            Unwrite(eq5c[0:2]),
            ReplacementTransform(eq5c[2:], eq4c[2:]),
            ReplacementTransform(eq4[0:2], eq4c[0:2]),
            ShrinkToCenter(eq4[2:]),
        )
        self.wait(1)
        ...
        eq10 = MathTex(
            r"\textbf{x} = \textbf{v}e^{i \omega t}", color=dark_blue, font_size=60
        )[0]
        eq10.shift(4 * UNIT * UP + 4 * UNIT * LEFT - eq10[0].get_bottom()[1] * UP)
        eq10[0].set_color(yellow)

        self.play(Write(eq10))
        self.wait(1)
        eq10a = MathTex("\Rightarrow", color=dark_blue, font_size=60)[0].shift(
            4.3 * UNIT * UP + UNIT * LEFT
        )

        eq11 = MathTex(
            r"\textbf{x}'' = - \omega^2 \textbf{v}e^{i \omega t}",
            color=dark_blue,
            font_size=60,
        )[0]
        ...
        eq12 = MathTex(
            r"\textbf{x}'' = -\omega^2 \textbf{x}", color=dark_blue, font_size=60
        )[0]
        eq12.shift(4 * UNIT * UP + 2.33 * UNIT * RIGHT - eq12[0].get_bottom()[1] * UP)
        eq12[0].set_color(yellow)
        eq12[7].set_color(yellow)
        self.play(
            ShrinkToCenter(eq11[7:]),
            ReplacementTransform(eq11[:7], eq12[:7]),
            ReplacementTransform(eq10[0].copy(), eq12[7:]),
        )
        self.wait(1)
        ...
        eq13 = MathTex(
            r"-\omega^2 \textbf{x} = A \textbf{x}", color=dark_blue, font_size=60
        )[0]
        eq13.shift(5.55 * UNIT * LEFT - eq13[1].get_bottom()[1] * UP)
        eq13[3].set_color(yellow)
        eq13[6].set_color(yellow)
        self.play(
            ReplacementTransform(eq12[4:].copy(), eq13[:4]),
            ReplacementTransform(eq0[3:], eq13[4:]),
            ShrinkToCenter(eq0[:3]),
        )

        eq13r = MathTex(
            r"A \textbf{x} = -\omega^2 \textbf{x}", color=dark_blue, font_size=60
        )[0]
        ...
        self.wait(1)
        self.play(
            ReplacementTransform(eq13[:4], eq13r[3:]),
            ReplacementTransform(eq13[4], eq13r[2]),
            ReplacementTransform(eq13[5:], eq13r[:2]),
        )

        self.wait(1)
        self.play(Unwrite(eq3), Unwrite(eq4c))
        self.play(
            self.camera.frame.animate.move_to(0),
            FadeOut(eq10),
            FadeOut(eq10a),
            FadeOut(eq12),
        )
