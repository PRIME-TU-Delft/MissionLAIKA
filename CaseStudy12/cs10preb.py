...
class Lecture10Scene(PrimeScene, MovingCameraScene):
    def construct(self):
        ...
        airplane = SVGMobject(
            "../assets/AirplaneTop.svg", stroke_color=dark_blue, stroke_width=6.3
        ).scale(1.93)
        airplane.z_index = -1

        self.play(
            *[sky.animate.set_opacity(0) for sky in skies],
            FadeIn(airplane),
            FadeOut(airplane_inside),
            run_time=2,
        )
        for sky in skies:
            sky.clear_updaters()
        ...
        tip_c1_big = attach_svg_tip_to_line(
            c1_big,
            red,
            line_end=e,
            scale=0.03,
        ...
        self.play(
            ReplacementTransform(tip_d1, tip_d1_big),
        ...
        self.play(
            ReplacementTransform(tip_c1, tip_c1_big),
        ...
        self.play(
            Group(
                ...
                tip_d1_big,
                tip_d2_big,
            ).animate.rotate(-30 * DEGREES, about_point=ORIGIN)
        )
        self.wait()

        vec0 = MathTex(
            r"\begin{bmatrix}1\\2 \\0\end{bmatrix}", color=red, font_size=20
        )[0]
        vec0.shift(vec0[3].get_bottom()[1] * DOWN + 3 * UNIT * LEFT)
        vec1 = MathTex(
            r"\begin{bmatrix}2\\-1 \\0\end{bmatrix}", color=blue, font_size=20
        )[0]
        vec1.shift(vec1[4].get_bottom()[1] * DOWN + 1 * UNIT * LEFT)
        vec2 = MathTex(
            r"\begin{bmatrix}1\\0 \\0\end{bmatrix}", color=yellow, font_size=20
        )[0]
        ...
        self.play(grid.background_lines.animate.set_stroke(width=0.5))
        self.wait()
        self.play(
            LaggedStart(
                ReplacementTransform(Group(c1_big, tip_c1_big), vec0),
        ...
        self.play(
            LaggedStart(
                ...
                GrowFromCenter(eq0[2:5]),
                GrowFromCenter(eq0[12:15]),
                lag_ratio=0.5,
            )
        )
        self.wait(1)
        ...
        self.play(
            LaggedStart(
                ...
                ReplacementTransform(
                    vec3[4:15], VGroup(*eq0[23:28], *eq0[29:34], eq0[35])
                ),
                GrowFromCenter(eq0[18:22]),
                GrowFromCenter(eq0[36:]),
                lag_ratio=0,
            )
        )
        self.wait(1)
