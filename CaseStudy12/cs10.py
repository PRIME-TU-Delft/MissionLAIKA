...
class Lecture10Scene(PrimeScene, ThreeDScene):
    def construct(self):
        ...
        self.set_camera_orientation(phi=cam_phi, theta=cam_theta, focal_distance=10000000)

        ar = [-2, 4, 1]
        axes = ThreeDAxes(x_range=ar, y_range=ar, z_range=ar, **{"axis_config": axes_defaults}).move_to(ORIGIN)
        ...
        e1_label = MathTex(r"\mathbf{e}_1", color=blue).next_to(e1.get_end(), RIGHT).rotate(90*DEGREES, axis=RIGHT).rotate(180 * DEGREES, axis=np.array([0,0,1]))[0]
        e2_label = MathTex(r"\mathbf{e}_2", color=blue).next_to(e2.get_end(), UP).rotate(90*DEGREES, axis=RIGHT).rotate(90 * DEGREES, axis=np.array([0,0,1]))[0]
        e3_label = MathTex(r"\mathbf{e}_3", color=blue).next_to(e3.get_end(), OUT).rotate(90*DEGREES, axis=RIGHT).rotate(135 * DEGREES, axis=np.array([0,0,1]))[0]

        # Add everything to the scene
        self.add(axes)
        self.wait()
        self.play(LaggedStart(GrowFromCenter(e1), GrowFromCenter(e2), GrowFromCenter(e3), lag_ratio=0.5))
        self.play(LaggedStart(GrowFromCenter(e1_label), GrowFromCenter(e2_label), GrowFromCenter(e3_label), lag_ratio=0.5))

        eq0 = MathTex(r"\mathbf{e}_1 \cdot \mathbf{e}_2 = "
                      r"\begin{bmatrix} 1\\ 0\\ 0 \end{bmatrix}"
                      r" \cdot \begin{bmatrix} 0\\ 1\\ 0 "
                      r"\end{bmatrix} = 0 ", color=dark_blue, font_size=30)[0]
        ...
        eq1 = MathTex(r"\mathbf{e}_2 \cdot \mathbf{e}_3 = "
        ...
        eq2 = MathTex(r"\mathbf{e}_3 \cdot \mathbf{e}_1 = "
        ...
        self.play(LaggedStart(ReplacementTransform(e1_label.copy(), eq0[0:2]), GrowFromCenter(eq0[2]), ReplacementTransform(e2_label.copy(), eq0[3:5]), Write(eq0[5:]), lag_ratio=0.5))
        self.play(LaggedStart(ReplacementTransform(e2_label.copy(), eq1[0:2]), GrowFromCenter(eq1[2]), ReplacementTransform(e3_label.copy(), eq1[3:5]), Write(eq1[5:]), lag_ratio=0.5))
        self.play(LaggedStart(ReplacementTransform(e3_label.copy(), eq2[0:2]), GrowFromCenter(eq2[2]), ReplacementTransform(e1_label.copy(), eq2[3:5]), Write(eq2[5:]), lag_ratio=0.5))

        self.wait()

        self.play(e1.animate.set_opacity(0.2), e2.animate.set_opacity(0.2), e3.animate.set_opacity(0.2), e1_label.animate.set_opacity(0.2), e2_label.animate.set_opacity(0.2), e3_label.animate.set_opacity(0.2))

        n1 = Arrow3D(start=axes.c2p(0, 0, 0), end=axes.c2p(0, 2, 0), color=red)
        n2 = Arrow3D(start=axes.c2p(0, 0, 0), end=axes.c2p(0, 0, 3), color=red)
        n1_label = MathTex(r"\begin{bmatrix} 0\\ 2\\ 0 \end{bmatrix}", font_size=30, color=red).next_to(n1.get_end(), UP).rotate(90 * DEGREES,
                                                                                         axis=RIGHT).rotate(
            90 * DEGREES, axis=np.array([0, 0, 1]))[0]
        n2_label = MathTex(r"\begin{bmatrix} 0\\ 0\\ 3 \end{bmatrix}", font_size=30, color=red).next_to(n2.get_end(), UP).rotate(90 * DEGREES,
                                                                                          axis=RIGHT).rotate(
            135 * DEGREES, axis=np.array([0, 0, 1]))[0]


        self.play(LaggedStart(GrowFromCenter(n1), GrowFromCenter(n2), GrowFromCenter(n1_label), GrowFromCenter(n2_label), lag_ratio=0.5))

        eq3 = MathTex(r"\begin{bmatrix} 0\\ 2\\ 0 \end{bmatrix} "
                      r"\cdot "
                      r"\begin{bmatrix} 0\\ 0\\ 3 \end{bmatrix}"
                      r" = 0", color=dark_blue, font_size=40)[0]
        ...
        self.play(LaggedStart(ReplacementTransform(n1_label.copy(), eq3[0:7]), GrowFromCenter(eq3[7]),
                              ReplacementTransform(n2_label.copy(), eq3[8:15]), Write(eq3[15:]), lag_ratio=0.5))
        self.wait(1)

        self.play(FadeOut(n1_label), FadeOut(n2_label), FadeOut(n1), FadeOut(n2), FadeOut(eq3))
        self.wait()

        self.play(e1.animate.set_opacity(1), e2.animate.set_opacity(1), e3.animate.set_opacity(1),
                  e1_label.animate.set_opacity(1), e2_label.animate.set_opacity(1),
                  e3_label.animate.set_opacity(1))
        self.wait()

        eq4 = MathTex(r"\mathbf{e}_1 \cdot \mathbf{e}_1 = "
                      r"\begin{bmatrix} 1\\ 0\\ 0 \end{bmatrix}"
                      r" \cdot \begin{bmatrix} 1\\ 0\\ 0 "
                      r"\end{bmatrix} = 1 ", color=dark_blue, font_size=30)[0]
        ...
        self.play(LaggedStart(ReplacementTransform(e1_label.copy(), eq4[0:2]), GrowFromCenter(eq4[2]),
                              ReplacementTransform(e1_label.copy(), eq4[3:5]), Write(eq4[5:]), lag_ratio=0.5))
        self.wait()
        ...
        self.play(LaggedStart(Write(grid), ShrinkToCenter(axes), ShrinkToCenter(e1), ShrinkToCenter(e2), ShrinkToCenter(e3), lag_ratio=0.5))

        eq5 = MathTex(r"\begin{bmatrix} \mathbf{e}_1 & \mathbf{e}_2 & \mathbf{e}_3 \end{bmatrix}", color=dark_blue, font_size=40)[0]
        ...
        self.move_camera(
            ...
            theta=-90 * DEGREES,
            run_time=3,
            added_anims=[ReplacementTransform(e1_label, eq5[1:3]), ReplacementTransform(e2_label, eq5[3:5]), ReplacementTransform(e3_label, eq5[5:7])]
        )
        ...
        eq6 = MathTex(r"\begin{bmatrix} 1 & 0 & 0\\ 0 & 1 & 0\\ 0 & 0 & 1 \end{bmatrix}", color=dark_blue, font_size=40)[0]
        eq6.shift(eq6[5].get_bottom()[1] * DOWN)

        self.play(LaggedStart(ReplacementTransform(eq5[0], eq6[0:2]), ReplacementTransform(eq5[1:3], VGroup(eq6[2], eq6[5], eq6[8])), ReplacementTransform(eq5[3:5], VGroup(eq6[3], eq6[6], eq6[9])), ReplacementTransform(eq5[5:7], VGroup(eq6[4], eq6[7], eq6[10])), ReplacementTransform(eq5[7], eq6[11:]), lag_ratio=0.5))
        self.wait()

        eq7 = MathTex(r"\begin{bmatrix} 0 & 0 & 1\\ 1 & 0 & 0\\ 0 & 1 & 0 \end{bmatrix}", color=dark_blue, font_size=40)[0]
        eq7.shift(eq7[5].get_bottom()[1] * DOWN)

        self.play(ReplacementTransform(eq6[0:2], eq7[0:2]),
                  ReplacementTransform(VGroup(eq6[3], eq6[6], eq6[9]), VGroup(eq7[2], eq7[5], eq7[8])),
                  ReplacementTransform(VGroup(eq6[4], eq6[7], eq6[10]), VGroup(eq7[3], eq7[6], eq7[9])),
                  ReplacementTransform(VGroup(eq6[2], eq6[5], eq6[8]), VGroup(eq7[4], eq7[7], eq7[10])),
        ...
        self.play(LaggedStart(eq7.animate.shift(3*UNIT*UP), FadeIn(eq7b), lag_ratio=0.5))
        self.wait()
        self.play(Write(eq8))
        self.wait()
        ...
        eq13 = MathTex(
            ...
            color=dark_blue, font_size=40)[0]
        eq13.shift(eq13[12].get_bottom()[1] * DOWN + eq13[0].get_left()[0] * LEFT + 3*UNIT*UP +4*UNIT*LEFT)
        ...
        eq14 = MathTex(
            ...
            color=dark_blue, font_size=40)[0]
        eq14.shift(eq14[12].get_bottom()[1] * DOWN + eq14[0].get_left()[0] * LEFT +4*UNIT*LEFT)
        ...
        eq15 = MathTex(
            ...
            color=dark_blue, font_size=40)[0]
        eq15.shift(eq15[12].get_bottom()[1] * DOWN + eq15[0].get_left()[0] * LEFT + 3*UNIT*DOWN +4*UNIT*LEFT)

        self.play(eq12.animate.shift(3*UNIT*LEFT))
        self.wait()

        self.play(LaggedStart(FadeOut(eq7b), FadeOut(VGroup(eq7[0:2], eq7[11:13])),
                              ...
                              FadeIn(VGroup(eq15[26:28], eq15[31:33])), lag_ratio=0.1
                              ))
        self.wait()

        self.play(LaggedStart(ReplacementTransform(eq12.copy(), eq13[:26]), Write(eq13[33:]),
                              ReplacementTransform(eq12.copy(), eq15[:26]), Write(eq15[33:]),
                              ReplacementTransform(eq12, eq14[:26]), Write(eq14[33:]),
                              lag_ratio=0.5
                              ))
        ...
        self.play(ReplacementTransform(eq13[26:28], eq16[26:28]),
                  FadeOut(VGroup(eq13[31:33].submobjects, eq14[26:28].submobjects, eq14[31:33].submobjects, eq15[26:28].submobjects,)),
        ...
        #UNIFY EQUAL SIGNS
        self.play(ReplacementTransform(VGroup(eq13[33], eq14[33], eq15[33]), eq16[39]),)

        # MAKE THE RESULT MATRIX OUT OF VECTORS
        self.play(ReplacementTransform(eq13[34:36], eq16[40:42]),
                  FadeOut(VGroup(eq13[46:].submobjects, eq14[34:36].submobjects, eq14[39:].submobjects,
                                 eq15[34:36].submobjects, )),
                  ReplacementTransform(eq15[45:], eq16[64:]),
                  ReplacementTransform(eq13[36:46], VGroup(eq16[42:47].submobjects, eq16[52:56].submobjects, eq16[61])),
                  ReplacementTransform(eq14[36:39], VGroup(eq16[47], eq16[56], eq16[62])),
                  ReplacementTransform(eq15[36:45], VGroup(eq16[48:52], eq16[57:61], eq16[63])),
                  )
        self.wait()
        ...
        self.play(LaggedStart(FadeOut(eq16[:40]), ReplacementTransform(eq16[40:], eq17), lag_ratio=0.5))
        self.wait()
