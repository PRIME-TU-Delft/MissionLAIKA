...
class Lecture10Scene(PrimeScene, MovingCameraScene):
    def construct(self):
        ...
        grid = NumberPlane(background_line_style={
            ...
            x_range=(-36, 36, 1),
            y_range=(-18, 18, 1),
        ).scale(UNIT).set_z_index(-2)
        self.play(FadeIn(grid))
        self.wait(1)

        rectangle = Rectangle(color=ManimColor('#FFFFFF'), height=12.0, width=7*UNIT, stroke_color=dark_blue, fill_color=ManimColor('#FFFFFF'), fill_opacity=1).shift((0.5 * config.frame_width + 1.5) * RIGHT).set_z_index(-1)
        ...
        square = Square(side_length=2*UNIT, color=red, stroke_color=red, fill_color=red).move_to(grid.get_center(), DL).set_z_index(-2)
        self.play(DrawBorderThenFill(square), self.camera.frame.animate.shift(5 * UNIT * RIGHT+ 2 *UNIT * UP), FadeIn(rectangle), FadeIn(dotted_rect))
        self.wait(1)
        ...
        eq00 = MathTex(
            ...
            color=dark_blue, font_size=40)[0].set_z_index(2)
        ...
        grid0 = NumberPlane(background_line_style={
            "stroke_color": dark_blue,
            "stroke_width": 1,
            "stroke_opacity": 0.15
        },
        ...
        self.play(ReplacementTransform(eq00[:5], eq0[:5]),
                  ShrinkToCenter(eq00[5:-1]), GrowFromCenter(eq0[5:-1]),
        ...
        self.play(ReplacementTransform(eq0[:5], eq00[:5]),
                  ShrinkToCenter(eq0[5:-1]), GrowFromCenter(eq00[5:-1]),
        ...
        self.play(ReplacementTransform(eq00[:5], eq1[:5]),
                  ShrinkToCenter(eq00[5:-1]), GrowFromCenter(eq1[5:-1]),
                  ReplacementTransform(eq00[-1], eq1[-1]),
                  ApplyMatrix(np.array([[1,2],[0,1]]), square), ApplyMatrix(np.array([[1,2],[0,1]]), grid1))
        self.wait(1)
        ...
        self.play(ReplacementTransform(eq1[:5], eq00[:5]),
                  ShrinkToCenter(eq1[5:-1]), GrowFromCenter(eq00[5:-1]),
        ...
        self.play(LaggedStart(Write(vec0), Write(vec1), Write(vec2), Write(vec3), lag_ratio=0.5))
        self.wait(1)

        vec4_label = MathTex(
            r"\begin{bmatrix} 0 \\ 2  \end{bmatrix}",
            color=blue, font_size=40)[0]
        ...
        vec8 = Line(start=ORIGIN, end=2*UNIT*UP + 4*UNIT*RIGHT, color=red).set_z_index(-2)
        vec8.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        vec9 = Line(start=2 * UNIT * UP + 4 * UNIT * RIGHT, end=2 * UNIT * UP + 6 * UNIT * RIGHT, color=red).set_z_index(-2)
        vec9.add_tip(tip_shape=StealthTip, tip_width=0.15, tip_length=0.15)
        vec10 = Line(start= 2 * UNIT * RIGHT, end=2 * UNIT * UP + 6 * UNIT * RIGHT, color=red).set_z_index(-2)
        ...
        self.play(ReplacementTransform(vec0.copy(), vec8),
        ...
        eq5b = MathTex(
            ...
            color=dark_blue, font_size=40
        )[0]
        eq5b.shift(center_of_rectangle)

        self.play(LaggedStart(ReplacementTransform(eq5[0], eq5b[0:4]),
                 ...
                 ReplacementTransform(eq5[8], eq5b[20:]),lag_ratio=0.25)
                 )
        self.wait(1)
        ...
        self.play(ReplacementTransform(eq5b[:13], eq5c[:13]),
        ...
        self.play(ReplacementTransform(eq6[0], eq6b[0:4]),
        ...
        eq6c = MathTex(
            r"\cos\theta = \frac{8}{2 \sqrt{20}}  ",
            color=dark_blue, font_size=40)[0]
        eq6c.shift(center_of_rectangle)

        self.play(ReplacementTransform(eq6b[15:], eq6c[0:4]),
        ...
        self.play(FadeOut(VGroup(eq6c, vec8_label, vec11_label, vec4_label, vec0, vec1, vec2, vec3, vec8, vec9, vec10, vec11)), eq1.animate.shift(3*DOWN))
        self.wait()
        ...
        self.play(ReplacementTransform(eq00[:5], eq7[:5]),
                  ShrinkToCenter(eq00[5:-1]), GrowFromCenter(eq7[5:-1]),
