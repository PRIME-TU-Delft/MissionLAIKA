...
class Plane(MovingCameraScene, PrimeScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                Transform(mobject=plane, target_mobject=m2, path_arc=0),
                ShrinkToCenter(line1),
                ShrinkToCenter(line2),
                lag_ratio=0.4,
                run_time=1.2,
            )
        )
        ...
        self.play(
            ...
            Transform(mobject=plane, target_mobject=m2, path_arc=0),
            ShrinkToCenter(inner_circle),
            wheel_text.animate.move_to(Point([0, -0.8, 0])),
            plane_text.animate.shift(0.2 * DOWN),
            run_time=1.2,
        )
        ...
        # springs
        spring2 = SVGMobject(
            "../assets/Spring.svg", stroke_color=blue, stroke_width=3
        ).scale(0.63)
        ...
        damping = SVGMobject(
            "../assets/damping.svg", stroke_color=dark_blue, stroke_width=3
        ).scale(0.63)
        damping.shift(0.88 * UP + 0.5 * LEFT)
        self.play(
            LaggedStart(
                spring2.animate.shift(RIGHT * 0.5), Write(damping), LaggedStart=0.9
            )
        )
        self.wait(1)
        gamma = MathTex("\gamma", font_size=60, color=dark_blue).move_to(
            damping.get_left() + LEFT
        )
        self.play(GrowFromPoint(gamma, damping.get_center()))
        self.wait(1)

        x2_text = MathTex("x_2", font_size=60, color=yellow).move_to(
            Point(m2.get_left() + [-2, 0, 0])
        )
        x1_text = MathTex("x_1", font_size=60, color=yellow).move_to(
            Point(m1.get_left() + [-2, 0, 0])
        )
        ...
        self.add(arrow2)
        self.play(
            gamma.animate.shift(0.4 * DOWN),
            k2_text.animate.shift(0.4 * DOWN),
            x2_text.animate.shift(0.4 * DOWN),
            m2group.animate.shift(0.8 * DOWN),
            arrow2.animate.stretch(8, 1, about_edge=UP),
            spring2.animate.stretch(0.34, dim=1, about_edge=DOWN),
            damping.animate.stretch(0.34, dim=1, about_edge=DOWN),
        )
        ...
        self.add(arrow1)
        self.play(
            gamma.animate.shift(0.8 * DOWN),
            k2_text.animate.shift(0.8 * DOWN),
            k1_text.animate.shift(0.2 * DOWN),
            x2_text.animate.shift(0.4 * DOWN),
            x1_text.animate.shift(0.4 * DOWN),
            m2m1group.animate.shift(0.8 * DOWN),
            arrow2.animate.stretch(2, 1, about_edge=UP),
            arrow1.animate.stretch(8, 1, about_edge=UP),
            spring.animate.stretch(0.34, dim=1, about_edge=DOWN),
        )

        # extend m2 mass
        self.play(
            gamma.animate.shift(0.4 * UP),
            k2_text.animate.shift(0.4 * UP),
            x2_text.animate.shift(0.4 * UP),
            m2group.animate.shift(0.8 * UP),
            arrow2.animate.stretch(0.5, 1, about_edge=UP),
        ...
        # extend both masses
        self.play(
            k2_text.animate.shift(0.8 * UP),
            gamma.animate.shift(0.8 * UP),
            k1_text.animate.shift(0.2 * UP),
            x2_text.animate.shift(0.4 * UP),
            x1_text.animate.shift(0.4 * UP),
            m2m1group.animate.shift(0.8 * UP),
            arrow2.animate.stretch(0.01, 1, about_edge=UP),
            arrow1.animate.stretch(0.01, 1, about_edge=UP),
            spring.animate.stretch(1 / 0.34, dim=1, about_edge=DOWN),
        )
        ...
        eq0 = MathTex("F_{\\text{spring}} = -kx", color=dark_blue)[0].shift(
            8 * RIGHT + 3 * UP
        )
        eq02 = MathTex("F_{\\text{gravity}}=-mg", color=dark_blue)[0].shift(
            8 * RIGHT + 2 * UP
        )
        ...
        eq1p = MathTex("F_{\\text{total}} = ma = mx'' ", color=dark_blue)[0].shift(
            8 * RIGHT
        )
        ...
        eq1p2 = MathTex("F_{\\text{total}} = mx'' ", color=dark_blue)[0].shift(
            8.1 * RIGHT
        )
        ...
        eq1p3 = MathTex("F_{\\text{damping}} = 0", color=dark_blue)[0].shift(
            7.45 * RIGHT + UP
        )
        ...
        eq1c = eq1.copy()
        eq1c2 = eq1.copy()
        eq2 = MathTex("m_1x_1'' = F_{\\text{total}_1}", color=dark_blue)[0].shift(
            4.35 * RIGHT + 1.15 * DOWN
        )
        eq3 = MathTex("m_2x_2'' = F_{\\text{total}_2}", color=dark_blue)[0].shift(
            4.35 * RIGHT + 1.85 * DOWN
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(eq1p[0:7], eq1p2[0:7]),
        ...
        self.wait()
        self.play(
            ReplacementTransform(eq1p2[0:6], eq1[5:]),
        ...
        self.play(
            (
                ReplacementTransform(eq1c2[0], eq2[0:2]),
                ReplacementTransform(eq1c[0], eq3[0:2]),
            ),
            (
                ReplacementTransform(eq1c2[1:4], eq2[2:6]),
                ReplacementTransform(eq1c[1:4], eq3[2:6]),
            ),
            (
                ReplacementTransform(eq1c2[4:11], eq2[6:14]),
                ReplacementTransform(eq1c[4:11], eq3[6:14]),
            ),
        )
        ...
        eq21 = MathTex(
            "m_1x_1'' = F_{\\text{gravity}_1} + F_{\\text{spring}_1} - F_{\\text{damping}}",
            color=dark_blue,
        )[0].shift(6.45 * RIGHT + 1.13 * DOWN)
        eq31 = MathTex(
            "m_2x_2'' = F_{\\text{gravity}_2} + F_{\\text{spring}_2} + F_{\\text{damping}}",
            color=dark_blue,
        )[0].shift(6.45 * RIGHT + 1.83 * DOWN)
        ...
        self.play(
            ReplacementTransform(eq2[0:7], eq21[0:7]),
            ReplacementTransform(eq3[0:7], eq31[0:7]),
            ReplacementTransform(eq2[7:], eq21[7:]),
            ReplacementTransform(eq3[7:], eq31[7:]),
        )
        ...
        self.wait()
        self.play(
            ShrinkToCenter(eq1p3p[9:14]),
            ReplacementTransform(eq1p3p[:9], eq1p3[:9]),
        ...
        self.play(
            ReplacementTransform(eq1, eq5[: len(eq1)]), GrowFromCenter(eq5[len(eq1) :])
        )

        eq22 = MathTex(
            "0 = F_{\\text{gravity}_1} + F_{\\text{spring}_1} - 0", color=dark_blue
        )[0].shift(6 * RIGHT + 1.15 * DOWN)
        eq32 = MathTex(
            "0 = F_{\\text{gravity}_2} + F_{\\text{spring}_2} + 0", color=dark_blue
        )[0].shift(6 * RIGHT + 1.85 * DOWN)

        self.play(
            LaggedStart(
                ShrinkToCenter(eq21[0:6]),
                ReplacementTransform(eq5[12].copy(), eq22[0]),
                ShrinkToCenter(eq31[0:6]),
                ReplacementTransform(eq5[12].copy(), eq32[0]),
        ...
        self.play(
            ...
            ReplacementTransform(eq6[4:7], eq7[1:4]),
            ShrinkToCenter(eq6[22]),
            ReplacementTransform(eq6[25:28], eq7[20:23]),
            ReplacementTransform(eq6[0], eq7[0]),
            ReplacementTransform(eq6[2], eq7[4]),
            ReplacementTransform(eq6[23], eq7[23]),
            ShrinkToCenter(eq6[3]),
            ShrinkToCenter(eq6[24]),
            ReplacementTransform(eq6[7:22], eq7[4:20]),
            ReplacementTransform(eq6[28:], eq7[23:]),
        )
        ...
        self.play(
            ReplacementTransform(eq7[11:13].copy(), eq8[16:18]),
            ShrinkToCenter(eq7[8:10]),
            GrowFromCenter(eq8[5]),
            GrowFromCenter(eq8[12]),
            ReplacementTransform(eq7[5:8], eq8[6:9]),
            ReplacementTransform(eq7[17:19], eq8[13:15]),
            ReplacementTransform(eq7[16], eq8[9]),
            ReplacementTransform(eq7[11:13], eq8[10:12]),
            ReplacementTransform(eq7[10], eq8[15]),
            ReplacementTransform(eq7[14:16], eq8[18:20]),
            ShrinkToCenter(eq7[13]),
            ShrinkToCenter(eq7[19]),
            ReplacementTransform(eq7[0:5], eq8[0:5]),
            ReplacementTransform(eq7[20:24], eq8[20:24]),
        )

        self.play(
            ShrinkToCenter(eq7[24]),
            ReplacementTransform(eq7[25:27].copy(), eq8[24:26]),
            ReplacementTransform(eq7[25:27], eq8[29:31]),
            ShrinkToCenter(eq7[27]),
            ShrinkToCenter(eq7[33]),
            ReplacementTransform(eq7[31:33], eq8[26:28]),
            ReplacementTransform(eq7[28:30], eq8[31:33]),
            ReplacementTransform(eq7[30], eq8[28]),
        )
